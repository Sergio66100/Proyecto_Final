...
// ═══════════════════════════════════════════════════════════════════
//  MICROSERVICIO PYTHON — URL y helper
// ═══════════════════════════════════════════════════════════════════
const MICRO_URL = "http://localhost:5001";

async function llamarMicro(path, method = "GET", body = null) {
  try {
    // node-fetch v3 es ESM; si usas CommonJS instala: npm install node-fetch@2
    const fetch = require("node-fetch");
    const opts  = { method, headers: { "Content-Type": "application/json" } };
    if (body) opts.body = JSON.stringify(body);
    const res = await fetch(`${MICRO_URL}${path}`, opts);
    return await res.json();
  } catch (e) {
    console.warn(`⚠️  Microservicio no disponible (${path}):`, e.message);
    return null;
  }
}
...
// ═══════════════════════════════════════════════════════════════════
//  PLANTILLAS DE EMAIL  (sin cambios respecto a tu versión)
// ═══════════════════════════════════════════════════════════════════
  ...
  <body style="margin:0;padding:0;background:#faf7f2;font-family:'Segoe UI',sans-serif;">
    <div style="max-width:520px;margin:32px auto;background:#fff;border-radius:16px;overflow:hidden;
      ...
      <div style="padding:24px 32px;text-align:center;border-top:1px solid #f0e6d3;margin-top:8px;">
        <p style="color:#aaa;font-size:0.82rem;margin:0;">Gracias por elegirnos 🙏 · Restaurante</p>
      </div>
    </div>
  </body></html>`;
}
...
// ═══════════════════════════════════════════════════════════════════
//  RECOMENDACIONES  ← NUEVO (proxy al microservicio Python)
// ═══════════════════════════════════════════════════════════════════

// GET /recomendaciones/top?n=8
// Devuelve los productos más pedidos → sección destacada del menú
app.get("/recomendaciones/top", async (req, res) => {
  const n    = req.query.n || 8;
  const data = await llamarMicro(`/recomendaciones/top?n=${n}`);

  if (data) return res.json(data);

  // Fallback si Python no está corriendo: primeros productos de la BD
  db.query("SELECT * FROM productos LIMIT 8", (err, result) => {
    res.json(err ? [] : result);
  });
});

// POST /recomendaciones/carrito
// Body: { ids: [1,3,5] }  → recomendaciones basadas en el carrito actual
app.post("/recomendaciones/carrito", async (req, res) => {
  const { ids } = req.body;
  const data = await llamarMicro("/recomendaciones/carrito", "POST", { ids: ids || [] });

  if (data) return res.json(data);
  res.json([]); // fallback silencioso
});
    ...
    (err, result) => {
      ...
      db.query("INSERT INTO items_pedido (id_pedido, id_producto, cantidad) VALUES ?", [valores],
        async (err2) => {
          ...
          // 🔄 Reentrenar modelo tras cada pedido real
          await llamarMicro("/recomendaciones/reentrenar", "POST");
...
// ═══════════════════════════════════════════════════════════════════
//  RESERVAS  (sin cambios respecto a tu versión)
// ═══════════════════════════════════════════════════════════════════
        ...
        (err2, solapadas) => {
          ...
          db.query(
            ...
            async (err3, result) => {
              ...
              if (notificacionesActivas) {
                try { await enviarEmailReserva({ email, nombre, fecha, hora, numPersonas: num_personas, notas, idReserva }); }
                catch (e) { console.error("⚠️ Email reserva:", e.message); }
              }
...
app.put("/reservas/:id/cancelar", (req, res) => {
  ...
  db.query("SELECT fecha, hora FROM reservas WHERE id_reserva = ?", [id], (err, result) => {
    ...
    const diferenciaHoras  = (fechaHoraReserva - new Date()) / (1000 * 60 * 60);

    if (diferenciaHoras < 2)
      return res.status(400).json({ success: false, mensaje: "No se puede cancelar con menos de 2 horas de antelación." });

    db.query("UPDATE reservas SET estado = 'cancelada' WHERE id_reserva = ?", [id], (err2) => {
      if (err2) return res.status(500).json({ success: false });
      res.json({ success: true });
    });
  });
